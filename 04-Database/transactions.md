...
Khi MVCC là chưa đủ và bạn cần kiểm soát nghiêm ngặt dữ liệu tránh Race Condition (ví dụ: rút tiền từ ví tài khoản đồng thời), bạn phải dùng Lock.
...
*   **Triển khai trong Spring Data JPA**:
    Sử dụng chú thích `@Lock` trên phương thức của Repository.
    ```java
    public interface AccountRepository extends JpaRepository<Account, Long> {
        
        // Tương đương câu lệnh SQL: SELECT ... FOR UPDATE
        @Lock(LockModeType.PESSIMISTIC_WRITE)
        @Query("SELECT a FROM Account a WHERE a.id = :id")
        Optional<Account> findByIdForUpdate(@Param("id") Long id);
    }
    ```
*   **Ứng dụng**: Thích hợp cho hệ thống có **tỷ lệ tranh chấp cao, yêu cầu tính đúng đắn tuyệt đối** (ví dụ: giỏ hàng thanh toán flash sale, số dư ví tiền tài khoản).

---

### 5.2. Khóa Lạc quan (Optimistic Locking)
*   **Ý nghĩa**: Bạn giả định xung đột ít khi xảy ra, vì vậy bạn không khóa gì cả khi đọc dữ liệu. Chỉ khi bạn ghi dữ liệu xuống (UPDATE), bạn mới kiểm tra xem dữ liệu có bị ai sửa đổi trước đó chưa bằng cách so khớp số phiên bản (version).
*   **Thực thi trong SQL**:
    ```sql
    ...
    ```
*   **Triển khai trong Spring Data JPA**:
    1. Thêm trường `@Version` vào Entity class:
    ```java
    @Entity
    public class Account {
        ...
        @Version // Hibernate tự động kiểm tra và tăng version này khi update
        private Long version;
    }
    ```
    2. Bắt ngoại lệ và xử lý Retry trong Service:
    ```java
    @Service
    public class AccountService {
        ...
        public void withdraw(Long accountId, Double amount) {
            try {
                ...
                // Xảy ra khi có một transaction khác đã update bản ghi trước và tăng version lên
                // Xử lý: Có thể ném ra một custom exception hoặc viết logic tự động retry 3 lần
                throw new BusinessException("Hệ thống đang bận xử lý giao dịch khác, vui lòng thử lại!");
            }
        }
    }
    ```
