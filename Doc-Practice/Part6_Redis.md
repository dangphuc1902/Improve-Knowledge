...
- **Ví dụ code Thực chiến (Hash):**
```java
// Redis Hash: Lưu và cập nhật từng thuộc tính của người chơi
public void updatePlayerHp(String playerId, int hp) {
    redis.hset("player:" + playerId, "hp", String.valueOf(hp));
}
```
...
- **Ví dụ code Thực chiến (ZSet):**
```java
// Redis Sorted Set: Thêm điểm vào Bảng xếp hạng
public void addScore(String playerId, double score) {
    redis.zadd("global-leaderboard", score, playerId);
}

// Lấy Top 10 cao thủ
public Set<Tuple> getTopPlayers() {
    return redis.zrevrangeWithScores("global-leaderboard", 0, 9);
}
```
...
### Cơ chế hoạt động:
1. **Lõi (Core)**: Redis thực hiện các lệnh (SET, GET, ...) bằng **duy nhất 1 thread**. Điểu này giúp loại bỏ hoàn toàn việc tranh chấp khóa (Locking) và Race Condition.
