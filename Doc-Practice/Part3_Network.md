...
- [x] Enrich **Part 2: Multi-threading** (ExecutorService, Locks, Atomic).
- [/] Enrich **Part 3: Network** (Netty Handlers, Protobuf).
...
```java
// Netty: Khởi tạo Server với LengthField Decoder
public void initChannel(SocketChannel ch) {
    ch.pipeline().addLast(new LengthFieldBasedFrameDecoder(1024, 0, 4, 0, 4));
    ch.pipeline().addLast(new PacketDecoder()); // Cửa ngõ giải mã
    ch.pipeline().addLast(new GameLogicHandler()); // Cửa ngõ xử lý logic
}
```
