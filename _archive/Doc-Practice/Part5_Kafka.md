...
## 2. Producer & Consumer: Logic vận hành
...
### Ví dụ code Thực chiến (Producer):
```java
// Kafka Producer: Gửi log trận đấu để xử lý hệ thống Nhiệm vụ (Quest)
public void sendMatchEndEvent(String playerId, MatchResult result) {
    ProducerRecord<String, MatchResult> record = new ProducerRecord<>("match-events", playerId, result);
    producer.send(record, (metadata, exception) -> {
        if (exception != null) logger.error("Gửi bài toán thất bại!");
    });
}
```
...
### Ví dụ code Thực chiến (Consumer):
```java
// Kafka Consumer: Nhận event để trao quà
@KafkaListener(topics = "match-events", groupId = "reward-service")
public void handleMatchEvent(MatchResult result) {
    if (result.isWin()) {
        rewardService.giveGold(result.getPlayerId(), 100);
    }
}
```
