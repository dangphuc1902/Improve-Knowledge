# 📊 Weekly Progress Tracker

> Cập nhật mỗi tối Chủ Nhật. Dùng để đánh giá tiến độ và điều chỉnh kế hoạch.

---

## 📅 Week 1 (16/06 - 22/06) — Arrays, Strings, HashMap, LinkedList, Stack

### LeetCode Progress
| # | Problem | Difficulty | Status | Time | Pattern | Notes |
|:--|:--------|:-----------|:-------|:-----|:--------|:------|
| 1 | Two Sum | Easy | ✅ | - | HashMap | - |
| 2 | Best Time to Buy/Sell Stock | Easy | ⬜ | - | Sliding Window | - |
| 3 | Contains Duplicate | Easy | ✅ | - | HashSet | - |
| 4 | Maximum Subarray | Medium | ⬜ | - | Kadane's/DP | - |
| 5 | Product of Array Except Self | Medium | ⬜ | - | Prefix/Suffix | - |
| 6 | Valid Anagram | Easy | ✅ | - | HashMap count | - |
| 7 | Group Anagrams | Medium | ⬜ | - | HashMap + Sort | - |
| 8 | Top K Frequent Elements | Medium | ⬜ | - | Bucket Sort/Heap | - |
| 9 | Longest Consecutive Sequence | Medium | ⬜ | - | HashSet | - |
| 10 | Reverse Linked List | Easy | ⬜ | - | Iterative/Recursive | - |
| 11 | Merge Two Sorted Lists | Easy | ⬜ | - | Two Pointers | - |
| 12 | Linked List Cycle | Easy | ⬜ | - | Fast/Slow Pointer | - |
| 13 | Valid Parentheses | Easy | ⬜ | - | Stack | - |
| 14 | Min Stack | Medium | ⬜ | - | Auxiliary Stack | - |
| 15 | Daily Temperatures | Medium | ⬜ | - | Monotonic Stack | - |

**Status Legend:** ⬜ Not started | 🟡 Attempted | ✅ Solved | 🔴 Need review

### Weekly Stats
- **Total solved this week:** ___ / 15
- **Easy:** ___ | **Medium:** ___ | **Hard:** ___
- **Average solve time (Medium):** ___ minutes
- **Weakest topic:** _______________

### Knowledge Topics Covered
- [ ] Big-O notation review
- [ ] Array operations & Two Pointers
- [ ] Sliding Window & Prefix Sum
- [ ] HashMap internals (Java): bucket, load factor, rehashing, treeification
- [ ] LinkedList: Singly vs Doubly, Fast/Slow pointers
- [ ] Stack, Queue, Monotonic Stack
- [ ] Java Collections Framework deep dive
- [ ] Java Memory Model: Stack vs Heap, GC basics

### English Practice
- [ ] Mon: Shadowing 1 tech video
- [ ] Tue: Read 1 tech blog article
- [ ] Wed: Listen to podcast 15 min
- [ ] Thu: Write project description in English
- [ ] Fri: Record 2-min self-intro
- [ ] Sat: Watch System Design video with EN subtitles
- [ ] Sun: Write 1 short blog post

### Self-Assessment (1-10)
| Area | Score | Notes |
|:-----|:------|:------|
| DSA Confidence | /10 | |
| Java Core | /10 | |
| English Speaking | /10 | |
| Motivation | /10 | |

### Adjustments for Next Week
- ___(ghi chú điều chỉnh)___

---

## 📅 Week 2 (23/06 - 29/06) — Trees, Binary Search, Heap

### LeetCode Progress
| # | Problem | Difficulty | Status | Time | Pattern | Notes |
|:--|:--------|:-----------|:-------|:-----|:--------|:------|
| 16 | Invert Binary Tree | Easy | ⬜ | - | DFS/BFS | - |
| 17 | Maximum Depth of Binary Tree | Easy | ⬜ | - | DFS | - |
| 18 | Same Tree | Easy | ⬜ | - | DFS | - |
| 19 | Level Order Traversal | Medium | ⬜ | - | BFS Queue | - |
| 20 | Validate BST | Medium | ⬜ | - | Inorder/Range | - |
| 21 | Binary Search | Easy | ⬜ | - | Binary Search | - |
| 22 | Search in Rotated Sorted Array | Medium | ⬜ | - | Modified BS | - |
| 23 | Find Min in Rotated Sorted Array | Medium | ⬜ | - | Modified BS | - |
| 24 | Koko Eating Bananas | Medium | ⬜ | - | BS on Answer | - |
| 25 | Search a 2D Matrix | Medium | ⬜ | - | BS on matrix | - |
| 26 | Kth Largest Element | Medium | ⬜ | - | QuickSelect/Heap | - |
| 27 | Merge K Sorted Lists | Hard | ⬜ | - | Min Heap | - |

### Knowledge Topics
- [ ] Stream API, Collectors, parallel streams
- [ ] Generics, wildcards, functional interfaces
- [ ] Spring Security: Filter chain, Auth flow
- [ ] Spring Data JPA: N+1, EntityGraph, Specification
- [ ] Spring Boot Testing: @SpringBootTest, TestContainers

### Weekly Stats
- **Total solved (cumulative):** ___ / 27
- **Self-assessment:** DSA: /10 | Java: /10 | English: /10

---

## 📅 Week 3 (30/06 - 06/07) — Graphs, DP Basics, Database

### LeetCode Progress
| # | Problem | Difficulty | Status | Time | Pattern | Notes |
|:--|:--------|:-----------|:-------|:-----|:--------|:------|
| 28 | Number of Islands | Medium | ⬜ | - | BFS/DFS Grid | - |
| 29 | Clone Graph | Medium | ⬜ | - | BFS + HashMap | - |
| 30 | Course Schedule | Medium | ⬜ | - | Topological Sort | - |
| 31 | Pacific Atlantic Water Flow | Medium | ⬜ | - | Multi-source DFS | - |
| 32 | Climbing Stairs | Easy | ⬜ | - | 1D DP | - |
| 33 | House Robber | Medium | ⬜ | - | 1D DP | - |
| 34 | Coin Change | Medium | ⬜ | - | Unbounded Knapsack | - |
| 35 | Longest Increasing Subsequence | Medium | ⬜ | - | DP + BS | - |
| 36 | Word Break | Medium | ⬜ | - | DP + Trie | - |
| 37-40 | Timed Assessment (3 bài) | Mixed | ⬜ | 90 min | - | - |

### Database Topics
- [ ] Window functions: ROW_NUMBER, RANK, LAG, LEAD
- [ ] EXPLAIN ANALYZE, Index types (B-tree, Hash, GIN)
- [ ] Normalization vs Denormalization trade-offs
- [ ] ACID, Isolation levels, MVCC, Deadlock
- [ ] CAP theorem, Eventual consistency

### ⭐ PHASE 1 CHECKPOINT
- [ ] 40+ bài LeetCode solved
- [ ] Giải 2 Medium trong 45 phút
- [ ] Java Collections/Concurrency tự tin
- [ ] SQL EXPLAIN đọc được
- [ ] **Decision: Tiếp Phase 2 hay extend 1 tuần?**

---

## 📅 Week 4-6 Template (Phase 2 — Deep Dive)

### Week 4 (07/07 - 13/07): DP Advanced + Cloud
_Copy format tuần 1-3 và fill problems: DP 2D, Greedy, Backtracking, Intervals_

### Week 5 (14/07 - 20/07): Trie + Advanced Graph + Distributed Systems
_Trie, Dijkstra, Topological Sort, Union Find, Hard problems_

### Week 6 (21/07 - 27/07): Review + Behavioral + Timed Practice
_STAR stories prep, LeetCode contests, full mock coding_

### ⭐ PHASE 2 CHECKPOINT
- [ ] 90+ bài LeetCode solved
- [ ] Medium < 25 phút average
- [ ] 6 STAR stories ready
- [ ] Cloud basics (AWS) understood
- [ ] K8s Pod/Service/Deployment understood

---

## 📅 Week 7-9 Template (Phase 3 — System Design)

### Week 7 (28/07 - 03/08): SD Framework + 5 Problems
_URL Shortener, Rate Limiter, Chat System, Notification, News Feed_

### Week 8 (04/08 - 10/08): SD Advanced + Start Apply
_E-commerce, Gaming Backend, Distributed Cache, Event-Driven, Search_

### Week 9 (11/08 - 17/08): Mock Interviews
_Full simulation rounds: Coding → SD → Behavioral → Tech Deep Dive_

### ⭐ PHASE 3 CHECKPOINT
- [ ] 110+ bài LeetCode
- [ ] 10 System Design problems mastered
- [ ] Mock interviews done
- [ ] CV updated, profiles active
- [ ] Applied to 3-5 warm-up companies

---

## 📅 Week 10-12 Template (Phase 4 — Interview Sprint)

### Active Interview Tracking
| Company | Applied Date | Status | Round | Feedback | Next Step |
|:--------|:-------------|:-------|:------|:---------|:----------|
| | | | | | |

### ⭐ FINAL CHECKPOINT
- [ ] Applied to 10-15 companies
- [ ] Completed 3-5 actual interviews
- [ ] Received ≥ 1 offer
- [ ] Negotiation completed
