...
2. **Lớp giao vận**: Cung cấp dịch vụ giao tiếp từ máy chủ đến máy chủ (TCP, UDP).
   ...
   - **TCP**: Hướng kết nối, đáng tin cậy, đảm bảo gửi và sắp xếp. Sử dụng bắt tay 3 bước.
   - **UDP**: Connectionless, best-effort, fast but unreliable. Good for video streaming or gaming.
   - **UDP**: Không kết nối, nỗ lực tối đa, nhanh nhưng không đáng tin cậy. Tốt cho truyền phát video hoặc chơi game.
...
## Practical Example
## Ví dụ thực tế
Let's see a simple TCP Server in Node.js, going below HTTP to work directly with TCP sockets.
Hãy xem một Máy chủ TCP đơn giản trong Node.js, đi xuống dưới HTTP để làm việc trực tiếp với các ổ cắm TCP.

```javascript
// Node.js TCP Server (net module)
// Máy chủ TCP Node.js (mô-đun net)
const net = require('net');

const server = net.createServer((socket) => {
    console.log('Client connected: ' + socket.remoteAddress);
    
    socket.write('Welcome to the generic TCP server!\n');
    
    socket.on('data', (data) => {
        console.log('Received: ' + data.toString());
        socket.write('Server Echo: ' + data.toString());
    });
...
## Exercises
## Bài tập
1. Build a basic TCP client in Java using `java.net.Socket` to connect to the Node.js TCP server provided above and send a message.
1. Xây dựng một máy khách TCP cơ bản bằng Java sử dụng `java.net.Socket` để kết nối với máy chủ TCP Node.js được cung cấp ở trên và gửi một tin nhắn.
